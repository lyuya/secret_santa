    ...
    <div className="bg-red-200 h-screen content-center">
        <div className="w-2/6 justify-self-center grid gap-y-3">
            <div className="w-full">
                ...
                <input className="w-full rounded-md h-10 p-2 text-xl shadow-md" value={name} onChange={($event) => setName($event.target.value)} />
            </div>
            <div>
                <div className="text-xl font-bold text-red-900">How many person will join this secret ?</div>
                <input className="w-full rounded-md h-10 p-2 text-xl shadow-md" type="number" value={nbPers} onChange={($event) => setNbPers(parseInt($event.target.value))} />
            </div>
            <div className="pb-10">
                ...
                <div className="pt-5">
                    ...
                    <ul>
                        {receivers && receivers.map((mail) => (
                            <li className={styles.emoji} key={mail}>
                                <Link href={"mailto:"+mail} className="text-red-900 font-bold">{mail}</Link>
                            </li>
                        ))}
                    </ul>
                </div>
            </div>
            <button className="justify-self-center rounded-full bg-red-700 p-2 w-60 text-white font-bold hover:bg-red-600">Create the secret !</button>
        </div>
    </div>
    </>)
}