...
export function organiseReceivers(receivers: string[], giftValue: number, name: string): MailRequest[] {
    ...
    return receivers.map((from) => {
        ...
        if (toBeSent.length === 2) {
            let indexOfLastOne = canBeSent.findIndex((mail) => mail === toBeSent[toBeSent.length - 1]);
